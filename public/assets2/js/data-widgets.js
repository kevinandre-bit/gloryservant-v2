$(function () {
 ...
 $(function(){
  const types                        = window.attendanceTypes;
  const weeklyCounts                 = window.attendanceWeeklyCounts;
  const dailyCounts                  = window.attendanceDailyCounts;
  const lastWeekDailyCounts          = window.attendanceLastWeekDailyCounts;

  const categories = [
    '8w ago','7w ago','6w ago','5w ago',
  ...
  types.forEach(type => {
    ...
    const $el  = $('#chart2-' + slug);
    if (!$el.length) return; // skip if missing

    // 1) yesterday vs same weekday last week
    const todayCount = dailyCounts[type]         || 0;
    const prevCount  = lastWeekDailyCounts[type] || 0;
    $('#count-'   + slug).text(todayCount.toLocaleString());
    const delta = todayCount - prevCount;
    const pct   = prevCount
      ? ((delta/prevCount)*100).toFixed(1)
      : '100.0';
    $('#percent-'    + slug).text(pct + '%');
    $('#trend-icon-' + slug).text(delta >= 0
      ? 'arrow_upward'
      : 'arrow_downward'
    );

    // 2) prepare sparkline data (reverse so “This week” is last)
    const raw = weeklyCounts[type] || Array(9).fill(0);
    ...
    new ApexCharts(document.querySelector('#chart2-' + slug), options)
      .render();
  });
});
