...
<main class="main-wrapper">
  <div class="main-content">
    ...
    {{-- Filters --}}
    <form class="card rounded-4 mb-3" method="get" action="{{ route('wellness.admin.followups.index') }}">
      <div class="card-body">
        <div class="row g-2 align-items-end">
          ...
          <div class="col-12 d-flex gap-2 mt-2">
            ...
            <a class="btn btn-outline-secondary" href="{{ route('wellness.admin.followups.index') }}">Reset</a>
          </div>
        </div>
      </div>
    </form>
            ...
            <tbody>
              @forelse($followups as $f)
                <tr>
                  ...
                  <td>{{ $f->group_name ?: '—' }}</td>
                  <td><span class="badge bg-light text-dark">{{ strtoupper($f->status) }}</span></td>
                  <td>
                    @php $sev = $f->severity; $cls = $sev==='high'?'bg-danger':($sev==='medium'?'bg-warning text-dark':'bg-success'); @endphp
                    <span class="badge {{ $cls }}">{{ strtoupper($sev) }}</span>
                  </td>
                  <td>{{ $f->leader_name }}</td>
                  ...
                  <td class="text-end">
                    ...
                    <form method="post" action="{{ route('wellness.admin.followups.assign',$f->id) }}" class="d-inline">
                      @csrf
              ...
              @empty
                <tr><td colspan="8" class="text-center text-muted p-4">No follow-ups match your filters.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
...
{{-- reuse the same “View” modal & JS from the leader page --}}
