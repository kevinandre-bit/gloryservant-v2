	...
	public function empList(Request $request) 
{
    ...
    $empList = DB::table('tbl_people')
        ->leftJoin('tbl_campus_data', 'tbl_campus_data.reference', '=', 'tbl_people.id')
        ->select(
            'tbl_people.*',
            'tbl_campus_data.*',
            'tbl_people.id as person_id',
            'tbl_campus_data.id as campus_row_id'
        )
        ->get();
	...
	public function empAtten(Request $request) 
	{
		...
		// 1) Compute today’s label
$today = date('M, d Y');

// 2) Pull attendance + campus/ministry
$empAtten = table::attendance()
    ->join('tbl_campus_data', 'tbl_people_attendance.reference', '=', 'tbl_campus_data.reference')
    ->select(
        'tbl_people_attendance.*',
        'tbl_campus_data.campus',
        'tbl_campus_data.ministry'
    )
    ->get();

// 3) (If you still need your people list for filters, keep this)
...
// 5) Time format & permission check
$tf = table::settings()->value('time_format');
if (permission::permitted('employee-attendance') === 'fail') {
    return redirect()->route('denied');
}

// 6) Pass $today into the view
return view(
    'admin.reports.report-employee-attendance',
    compact('empAtten', 'employee', 'tf', 'today', 'role','ministries','campuses')
);
	}
	...
	public function empLeaves(Request $request) 
	{
        ...
        $role       = strtoupper(str_replace('-', ' ', trim($roleName)));  
		$today = date('M, d Y');
		$employee = table::people()->join('tbl_campus_data', 'tbl_people.id', '=', 'tbl_campus_data.reference')->where('tbl_people.employmentstatus', 'Active')->get();
		$empLeaves = table::leaves()->get();
		table::reportviews()->where('report_id', 3)->update(array('last_viewed' => $today));
    
		//return view('admin.reports.report-employee-leaves', compact('empLeaves', 'employee', 'role','ministries','campuses'));
	}
	...
	public function orgProfile(Request $request) 
	{
		...
		$today = date('M, d Y');
		$ed = table::people()->join('tbl_campus_data', 'tbl_people.id', '=', 'tbl_campus_data.reference')->where('tbl_people.employmentstatus', 'Active')->get();
		...
		foreach ($ed as $c) { $comp[] = $c->campus; $dcc = array_count_values($comp); }
		$cc = ($dcc == null) ? null : implode($dcc, ', ') . ',' ;

		foreach ($ed as $d) { $dept[] = $d->ministry; $dpc = array_count_values($dept); }
		...
		$orgProfile = table::campusdata()->get();
	...
	public function empBday(Request $request) 
	{
		...
		$today = date('M, d Y');
		$empBday = table::people()->join('tbl_campus_data', 'tbl_people.id', '=', 'tbl_campus_data.reference')->get();
		table::reportviews()->where('report_id', 7)->update(['last_viewed' => $today]);

		return view('admin.reports.volunteer-birthday', compact('empBday'));
	}
