...
class ExportsController extends Controller
{
	...
	function campus(Request $request) 
	{
		...
		foreach ($c as $d) 
		{
		    Storage::prepend($file, $d->id .','. $d->campus);
		}

		Storage::prepend($file, '"ID"' .','. 'campus');

		return Storage::download($file);
    }

	function ministry(Request $request) 
	{
		...
		$date = date('Y-m-d');
        $time = date('h-i-sa');
		$file = 'ministries-'.$date.'T'.$time.'.csv';

		Storage::put($file, '', 'private');

		foreach ($d as $i) 
		{
		    Storage::prepend($file, $i->id .','. $i->ministry);
		}

		Storage::prepend($file, '"ID"' .','. 'ministry');
	...
	function jobtitle(Request $request) 
	{
		...
		Storage::prepend($file, '"ID"' .','. 'ministry' .','. 'ministry CODE');
	...
	{
		if (permission::permitted('reports')=='fail'){ return redirect()->route('denied'); }
		$c = table::people()->join('tbl_campus_data', 'tbl_people.id', '=', 'tbl_campus_data.reference')->get();
		...
		foreach ($c as $d) 
		{
		    Storage::prepend($file, $d->idno .','. $d->lastname.' '.$d->firstname.' '.$d->mi .','. $d->ministry .','. $d->jobposition .','. $d->birthday .','. $d->mobileno);
		}

		Storage::prepend($file, '"ID"' .','. 'EMPLOYEE NAME' .','. 'ministry' .','. 'POSITION' .','. 'BIRTHDAY' .','. 'MOBILE NUMBER' );
