@extends('layouts.default')
    ...
    @section('meta')
        <title>Job Titles | Glory Servant</title>
                ...
                <div class="box-body">
                    ...
                    <form id="add_jobtitle_form" action="{{ url('fields/jobtitle/add') }}" class="ui form" method="post" accept-charset="utf-8">
                        ...
                        <div class="field">
                            ...
                            <select name="ministry" class="ui search dropdown getdeptcode">
                                <option value="">Select ministry</option>
                                @isset($d)
                                    @foreach ($d as $dept)
                                    <option value="{{ $dept->ministry }}" data-id="{{ $dept->id }}"> {{ $dept->ministry
                                        }}</option>
                                    @endforeach
                                @endisset
                            </select>
    ...
    $('.ui.dropdown.getdeptcode').dropdown({
        onChange: function (value, text, $selectedItem) {
            $('select[name="ministry"] option').each(function () {
                if ($(this).val() == value) {
